...
func (q *TokenQuery) GetByUserID(userID uint) (Token, error) {
	var token Token
...
// Update 更新令牌信息
func (q *TokenQuery) Update(userID uint, token Token) (Token, error) {
	err := q.db.WithContext(q.ctx).Model(&Token{}).Where("user_id = ?", userID).Updates(&token).Error
	return token, err
}

// Delete 删除令牌记录
func (q *TokenQuery) Delete(userID uint) error {
