...
func (x *UpdateUserRoleReq) fastReadField3(buf []byte, _type int8) (offset int, err error) {
	x.Token, offset, err = fastpb.ReadString(buf, _type)
	return offset, err
}
...
SkipFieldError:
	return offset, fmt.Errorf("%T cannot parse invalid wire-format data, error: %s", x, err)
ReadFieldError:
	return offset, fmt.Errorf("%T read field %d '%s' error: %s", x, number, fieldIDToName_UpdateUserRoleResp[number], err)
}
...
func (x *UpdateUserRoleReq) fastWriteField3(buf []byte) (offset int) {
	if x.Token == "" {
		return offset
	}
...
func (x *UpdateUserRoleReq) sizeField3() (n int) {
	if x.Token == "" {
		return n
	}
